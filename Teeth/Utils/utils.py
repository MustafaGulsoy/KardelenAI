...
def pair_teeth_processes(numbering_keeps, process_keeps):
    paired = {}
    ...
    for process_class_id, (process_result, _, process_box) in process_keeps.items():
        ...
        tooth_results["result"][process_class_id]["status"] = "Sağlam diş"
        if process_name in multiple_match_processes:
            ...
            for result in collisions:
                ...
                tooth_results['result'][int(result['tooth_id'])].append({
            ...
            tooth_results['result'][closest_tooth[0]]["annotations"].append({
                'process_name': process_name,
                "x_start": x1,
                "y_start": y1,
                "x_end": x2,
    ...
    #         paired[str(closest_tooth[0])] = process_name
    #
    # for tooth_id, (_, __, ___) in numbering_keeps.items():
    #     if numbering_keeps[1][0].names[tooth_id] in list(paired.keys()):
    #         tooth_results[numbering_keeps[1][0].names[tooth_id]] = paired[numbering_keeps[1][0].names[tooth_id]]
    #     else:
    #         tooth_results[numbering_keeps[1][0].names[tooth_id]] = 'Sağlam'
